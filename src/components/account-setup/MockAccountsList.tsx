...
interface MockAccountsListProps {
  accounts: MockAccount[];
  selectedAccounts: Set<string>;
  onToggleAccount: (accountId: string) => void;
}

export function MockAccountsList({ accounts, selectedAccounts, onToggleAccount }: MockAccountsListProps) {
  return (
        ...
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-lg font-medium">
              {account.name}
              <span className="ml-2 text-sm text-gray-500">(Mock Data)</span>
            </CardTitle>
            <Button
